...
// List of established legitimate Indian news sources
...
// List of government and educational domains in India
...
// Keywords often used in misinformation sites in Indian context
...
async function fetchUrlContent(url) {
  try {
    ...
    // Using a simple regex to extract the main text content
    const content = response.data.replace(/<[^>]*>?/gm, ' ').replace(/\s+/g, ' ').trim();
    return content;
  } catch (error) {
  ...
  try {
    ...
    const fallbackResponse = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: systemRole + ' (Note: This is a fallback analysis as the primary service is unavailable.)'
        },
        {
          role: 'user',
          content: prompt
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: system + ' IMPORTANT: Format your response as valid JSON.'
        },
        {
          role: 'user',
          content: prompt
    ...
    try {
      ...
      if (jsonMatch) {
        return JSON.parse(jsonMatch[0].startsWith('{') || jsonMatch[0].startsWith('[') 
          ? jsonMatch[0] 
          : jsonMatch[1]);
      } else {
...
async function detectMisinformation(content, perplexityResponse, openaiResponse) {
  try {
    // First check if it's too short to analyze meaningfully
    ...
    // Look for patterns indicating misinformation in AI responses
    const combinedAnalysis = perplexityResponse + " " + openaiResponse;
    
    // Keywords that strongly suggest factual issues
    const misinfoKeywords = [
    ...
    // Count how many misinformation-related keywords appear
    let keywordHits = 0;
    let matchedKeywords = [];
    ...
    // Calculate a misinformation score based on keyword density and other factors
    const misinfoConfidence = Math.min(100, keywordHits * 20); // Scale up to 100%
    const MAX_CONTENT_LENGTH = 1000; // Adjust based on needs
    const trimmedContent = content.length > MAX_CONTENT_LENGTH ? content.substring(0, MAX_CONTENT_LENGTH) + "..." : content;
    
    // Additional analysis with GPT to double-check
    const verificationResponse = await openai.chat.completions.create({
      model: 'gpt-4o',
      messages: [
        {
          role: 'system',
          content: 'You are a fact-checking expert. Analyze the following content and determine if it contains misinformation, answering only YES or NO.'
        },
        {
          role: 'user',
    ...
    // Get the simple yes/no response
...
export const analyzeUrl = async (req, res) => {
  try {
    ...
    // Step 1.5: NEW - Verify website trust with India-specific focus
    ...
    // Step 2: Analyze with Perplexity for fact-checking
    ...
    // Step 3: Analyze with OpenAI for deepfake/manipulation detection
    ...
    const openaiResponse = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a media analysis expert specialized in identifying patterns of misinformation, manipulated media, and bias in content, with particular expertise in Indian media and information ecosystem.'
        },
        {
          role: 'user',
          content: `Analyze this content from URL ${url} for manipulation, bias, and signs of generated or misleading information. If the content relates to India, consider common patterns of misinformation in Indian media:\n\n${content.substring(0, 4000)}`
        }
      ],
      max_tokens: 1000
    });
    
    // Step 4: Analyze source reputation with OpenAI - ENHANCED with India-specific trust signals
    ...
    // Step 6: First check for misinformation
    ...
    // Step 7: Get truth score and factors from OpenAI
    ...
    if (misinfoCheck.isMisinformation) {
      ...
      if (factualIndex >= 0) {
        // Significantly reduce the factual accuracy score
        factorsData.factors[factualIndex].score = Math.max(5, 100 - factorsData.factors[factualIndex].score);
      }
      
      // Calculate new truthScore weighted heavily by misinformation detection
      const misinfoWeight = 0.7; // How much weight to give to misinformation detection
      const originalWeight = 0.3; // How much weight to give original score
      
      // A high misinfoConfidence means low truthScore
    ...
    // NEW - Combine content trust score with technical trust score
    const originalTruthScore = factorsData.truthScore;
    
    // Apply stronger weight to India-specific trust signals
    ...
    // Update source credibility factor
    ...
    if (credibilityIndex >= 0) {
      // Blend existing source credibility with technical trust
      ...
      // Add an Indian source credibility factor if none exists
    ...
    // Boost factor for government sources 
    ...
    // Step 8: Get a summary from OpenAI
    ...
    const summaryResponse = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'Provide a concise 3-4 sentence summary of the analysis, highlighting key findings about the content credibility, factual accuracy, and potential issues. If the content relates to India, mention this specifically.'
        },
        {
          role: 'user',
          content: `Perplexity analysis: ${perplexityResponse}\n\nOpenAI analysis: ${openaiResponse.choices[0].message.content}\n\nSource reputation: ${reputationResponse.choices[0].message.content}\n\nTechnical trust analysis: ${JSON.stringify(trustSignals, null, 2)}`
        }
      ],
      max_tokens: 300
    });
...
export const analyzeText = async (req, res) => {
  try {
    ...
    // Create a cache key based on a hash of the text
    ...
    // Step 1: Analyze with Perplexity for fact-checking
    ...
    // Step 2: Analyze with OpenAI for manipulation detection
    ...
    const openaiResponse = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a media analysis expert specialized in identifying patterns of misinformation, AI-generated content, and bias. Analyze the provided content for these issues.'
        },
        {
          role: 'user',
          content: `Analyze this content for manipulation, bias, and signs of generated or misleading information:\n\n${text.substring(0, 4000)}`
        }
      ],
      max_tokens: 1000
    });
    
    // Step 3: Extract key facts and metadata using GPT
    ...
    // Step 4: First check for misinformation
    console.log('Checking for misinformation');
    const misinfoCheck = await detectMisinformation(
      text,
      perplexityResponse,
    ...
    // Step 5: Get truth score and factors from OpenAI
    console.log('Calculating truth score and factors');
    const factorsFallback = {
      truthScore: 50,
      factors: [
    ...
    // Step 6: Get a summary from OpenAI
    console.log('Generating summary');
    const summaryResponse = await openai.chat.completions.create({
      model: 'gpt-4o',
      messages: [
        {
          role: 'system',
          content: 'Provide a concise 3-4 sentence summary of the analysis, highlighting key findings about the content credibility, factual accuracy, and potential issues.'
        },
        {
          role: 'user',
          content: `Perplexity analysis: ${perplexityResponse}\n\nOpenAI analysis: ${openaiResponse.choices[0].message.content}`
...
export const analyzeImage = async (req, res) => {
  try {
    ...
    // Use a cache key based on the file name and size
    ...
    const visionResponse = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert in detecting image manipulation, deepfakes, and visual misinformation. Analyze the provided image in detail for signs of manipulation."
        },
        {
          role: "user",
          content: [
    ...
    // Step 2: Get deeper analysis for specific elements
    ...
    // Step 3: Calculate factors and truth score
    console.log('Calculating truth score and factors');
    const factorsFallback = {
      truthScore: 50,
      factors: [
    ...
    const summaryResponse = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'Provide a concise 3-4 sentence summary of the image analysis, highlighting key findings about authenticity and potential manipulation.'
        },
        {
          role: 'user',
          content: `Image analysis: ${visionResponse.choices[0].message.content}`
        }
      ],
      max_tokens: 300
    });
    
    // Step 5: Save image for reference (optional)
    ...
    const result = {
      ...
      imagePath: `/uploads/analyzed/${fileName}` // Store path for frontend reference
    };
    
    // Cache the result and clean up the original uploaded file
    analysisCache.set(cacheKey, result);
...
export const analyzeVideo = async (req, res) => {
  try {
    ...
    // Step 1: Get basic analysis from OpenAI based on file information
    ...
    const basicResponse = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert in detecting video manipulation and deepfakes. Based on the limited information provided, estimate the likelihood of manipulation."
        },
        {
          role: "user",
          content: `Analyze this video file information:\nFilename: ${videoFile.originalname}\nSize: ${videoFile.size} bytes\nMIME type: ${videoFile.mimetype}`
        }
      ],
      max_tokens: 500
    });
    
    // Step 2: Generate mock detailed analysis
    // In a real implementation, you would use specialized video analysis services
    ...
    const summaryResponse = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'Provide a concise 3-4 sentence summary of the video analysis, acknowledging the limitations of the analysis and highlighting potential authenticity concerns.'
        },
        {
          role: 'user',
          content: `Video analysis: ${basicResponse.choices[0].message.content}`
        }
      ],
      max_tokens: 300
    });
    
    // Step 5: Save video for reference (optional)
    const uploadDir = path.join(__dirname, '../uploads/analyzed');
    if (!fs.existsSync(uploadDir)) {
      fs.mkdirSync(uploadDir, { recursive: true });
    }
    ...
    const result = {
      ...
      videoPath: `/uploads/analyzed/${fileName}` // Store path for frontend reference
    };
